...
public class JobContainer extends AbstractContainer {
    ...
    private Map<String,Writer.Job> jobWriter;
...
//             communication.setState(State.FAILED);
    ...
    private Map<String,Writer.Job> preCheckWriterInit(JobPluginCollector jobPluginCollector) {
    	Map<String,Writer.Job> jobs = new HashMap<String, Writer.Job>();
        this.writerPluginName = this.configuration.getStrings(
                CoreConstant.DATAX_JOB_CONTENT_WRITER_NAME);
        for(String writerName:this.writerPluginName){
	        classLoaderSwapper.setCurrentThreadClassLoader(LoadUtil.getJarLoader(
	                PluginType.WRITER, writerName));
	...
	private void preCheckWriter() {
		for (String writerName : this.writerPluginName) {
			classLoaderSwapper
					.setCurrentThreadClassLoader(LoadUtil.getJarLoader(PluginType.WRITER, writerName));
			LOG.info(String.format("DataX Writer.Job [%s] do preCheck work .", writerName));
			this.jobWriter.get(writerName).preCheck();
			classLoaderSwapper.restoreCurrentThreadClassLoader();
		}
	}
...
//        LOG.info("Scheduler configuration ={}",this.configuration);
        LOG.info("Scheduler starts [{}] taskGroups.", taskGroupConfigs.size());
//        for (Configuration configuration:taskGroupConfigs){
//            LOG.info("oneGroup Conf ={}", configuration.toString());
//        }

//        for (Configuration taskGroupConfiguration : taskGroupConfigs) {
//             for (Configuration configuration:taskGroupConfiguration.getListConfiguration(
//                     CoreConstant.DATAX_JOB_CONTENT)) {
//                             LOG.info("oneJOb Conf ={}", configuration.toString());
//
//             }
//        }
...
//        for(String key:jobWriter.keySet()){
//            Writer.Job jobWriter = this.jobWriter.get(key);
//            LOG.debug("destroy= {}", jobWriter.toString());
//            if (jobWriter != null) {
//                jobWriter.destroy();
//            }
//        }
//        this.jobWriter = null;
    ...
    private void logStatistics() {
        ...
        LOG.info(String.format(
                ...
                String.valueOf(recordSpeedPerSecond)
                        + "rec/s", "读出记录总数",
                String.valueOf(CommunicationTool.getTotalReadRecords(communication)/this.jobWriter.size()),
                "读写失败总数",
     ...
     * @return
     */
    private Map<String,Writer.Job> initJobWriter(JobPluginCollector jobPluginCollector) {
    	Map<String,Writer.Job>  writers=new HashMap<String, Writer.Job>();
        this.writerPluginName = this.configuration.getStrings(
                CoreConstant.DATAX_JOB_CONTENT_WRITER_NAME);
//        LOG.info(this.writerPluginName.toString());
        for(String writerName:this.writerPluginName){
            LOG.info("initJobWriter writerName ={}",writerName);
        	 classLoaderSwapper.setCurrentThreadClassLoader(LoadUtil.getJarLoader(
                     PluginType.WRITER, writerName));

             Writer.Job jobWriter = (Writer.Job) LoadUtil.loadJobPlugin(
                     PluginType.WRITER, writerName);
//             LOG.info("Configuration = {}", this.configuration.getConfiguration(
//                     CoreConstant.DATAX_JOB_CONTENT_WRITER_PARAMETER,writerName));
             // 设置writer的jobConfig
             jobWriter.setPluginJobConf(this.configuration.getConfiguration(
                     CoreConstant.DATAX_JOB_CONTENT_WRITER_PARAMETER,writerName));
             ...
             jobWriter.init();
             classLoaderSwapper.restoreCurrentThreadClassLoader();
             writers.put(writerName, jobWriter);
        }
    ...
    private void prepareJobWriter() {
    	for(String writerName:writerPluginName){
    		  classLoaderSwapper.setCurrentThreadClassLoader(LoadUtil.getJarLoader(
    	                PluginType.WRITER, writerName));
    	        LOG.info(String.format("DataX Writer.Job [%s] do prepare work .",writerName));
    	        this.jobWriter.get(writerName).prepare();
//    	        LOG.info(this.jobWriter.get(writerName).getPluginJobConf().toString());
    	        classLoaderSwapper.restoreCurrentThreadClassLoader();
    	}
    ...
    private List<Configuration> doWriterSplit(int readerTaskNumber) {
    	List<Configuration> writerSlicesConfigs = new ArrayList<Configuration>();
    	List<List<Configuration>> multitypeWriterConfList = new ArrayList<List<Configuration>>();

    	//1.进行WRITER split  Configuration
        for(String writerName:writerPluginName){
            classLoaderSwapper.setCurrentThreadClassLoader(LoadUtil.getJarLoader(
                    PluginType.WRITER, writerName));
            Writer.Job tmpJob =this.jobWriter.get(writerName);
            List<Configuration> oneTypeWriterConfList = this.jobWriter.get(writerName).split(readerTaskNumber);
            if (oneTypeWriterConfList == null || oneTypeWriterConfList.size() <= 0) {
                throw DataXException.asDataXException(
                        FrameworkErrorCode.PLUGIN_SPLIT_ERROR,
                        "writer切分的task不能小于等于0");
            }
            LOG.info("DataX Writer.Job [{}] splits to [{}] tasks.",
                    writerName, oneTypeWriterConfList.size());
//            LOG.info("after split, one conf= {}", oneTypeWriterConfList.get(0).toString());

            multitypeWriterConfList.add(oneTypeWriterConfList);
        }
        //2.按照  readerTaskNumber 循环添加 不同writer 的Configuration
        for(int i=0;i<readerTaskNumber;i++){
            for(int j=0;j<this.writerPluginName.length;j++){
                writerSlicesConfigs.add(multitypeWriterConfList.get(j).get(i));
            }
        }
        classLoaderSwapper.restoreCurrentThreadClassLoader();
...
//        for(String writerName:writerPluginName){
//    	    LOG.info("doWriterSplit writerName ={} ",writerName);
//    		 classLoaderSwapper.setCurrentThreadClassLoader(LoadUtil.getJarLoader(
//    	                PluginType.WRITER, writerName));
//             Writer.Job tmpJob =this.jobWriter.get(writerName);
//    		 LOG.info(tmpJob.getPluginName());
//    		 LOG.info(tmpJob.getPluginJobConf().toString());
//    		 LOG.info(tmpJob.getPeerPluginName());
//    		 LOG.info("****************************************************");
//
//    	        writerSlicesConfigs.addAll(this.jobWriter.get(writerName)
...
//    	        }
//    	        LOG.info("DataX Writer.Job [{}] splits to [{}] tasks.",
//    	                writerName, writerSlicesConfigs.size());
//    	        LOG.info(writerSlicesConfigs.get(writerSlicesConfigs.size()-1).toString());//最后面的应该是es writer的配置
//    	        classLoaderSwapper.restoreCurrentThreadClassLoader();
//    	}
...
//        }
        ...
        for (int i = 0; i < readerTasksConfigs.size(); i++) {
            ...
            /**
            根据寻址添加writer 参数，避免引入 internal不必要的路径关键字
             */
            for(int j=0;j<writerPluginName.length;j++){
                String path = String.format("writer[%s].",j);
                taskConfig.set(path + "name",this.writerPluginName[j]);
                taskConfig.set(path + "parameter",writerTasksConfigs.get(j));
            }
            LOG.debug("after set ={}",taskConfig.toString());
    	...
    	for(String writerName:writerPluginName){
    		  classLoaderSwapper.setCurrentThreadClassLoader(LoadUtil.getJarLoader(
    	        ...
    	        LOG.info("DataX Writer.Job [{}] do post work.",
    	        		writerName);
    	        this.jobWriter.get(writerName).post();
    	        classLoaderSwapper.restoreCurrentThreadClassLoader();
    	}
