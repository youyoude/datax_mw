...
public final class ConfigParser {
    ...
    public static Configuration parse(final String jobPath) {
        ...
        String[] writerPluginNames = configuration.getStrings(
                CoreConstant.DATAX_JOB_CONTENT_WRITER_NAME);

        String[] preHandlerNames = configuration.getStrings(
                CoreConstant.DATAX_JOB_PREHANDLER_PLUGINNAME);

        String[] postHandlerNames = configuration.getStrings(
                CoreConstant.DATAX_JOB_POSTHANDLER_PLUGINNAME);

        Set<String> pluginList = new HashSet<String>();
        pluginList.add(readerPluginName);
        for(String writerPluginName :writerPluginNames){
        	pluginList.add(writerPluginName);
        }

        if(preHandlerNames!=null&&preHandlerNames.length>0) {
        	 for(String preHandlerName :preHandlerNames){
             	pluginList.add(preHandlerName);
             }
        }
        if(postHandlerNames!=null&&postHandlerNames.length>0) {
        	 for(String postHandlerName :postHandlerNames){
        ...
        try {
            ...
            LOG.warn(String.format("插件[%s,%s]加载失败，1s后重试... Exception:%s ", readerPluginName, writerPluginNames, e.getMessage()));
            try {
                Thread.sleep(1000);
    ...
    public static void main(String[] args) {
        ...
        String jobContent = getJobContent("/Users/mayunqi/Desktop/aa.json");
        Configuration config = Configuration.from(jobContent);

        String[] strings = config.getStrings(
                CoreConstant.DATAX_JOB_CONTENT_WRITER_NAME);

        String DATAX_JOB_CONTENT_WRITER_PARAMETER = "job.content[0].writer";

        for(String  a :strings){
            System.out.println(a );
            System.out.println(config.getConfiguration(DATAX_JOB_CONTENT_WRITER_PARAMETER,a));

        }


//        config.set(CoreConstant.DATAX_JOB_CONTENT_WRITER_PARAMETER + ".dryRun", true);
