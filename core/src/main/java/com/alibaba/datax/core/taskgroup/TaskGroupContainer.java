...
public class TaskGroupContainer extends AbstractContainer {
	...
	public TaskGroupContainer(Configuration configuration) {
        super(configuration);
//        LOG.info("init TaskGroupContainer {}" ,configuration.toString());
...
//        private Channel channel;
        ...
        private List<WriterRunner> writerRunners=new ArrayList<WriterRunner>();
        //存TaskExecutor 所初始化的 channel key
        private List<String>  channelGroupKeyList =new ArrayList<String>();
        /**
        ...
        public TaskExecutor(Configuration taskConf, int attemptCount) {
            LOG.info("init channelGroupKeyList ={}",this.channelGroupKeyList.toString());
             ...
             * 生成writerThread
             */
            List<AbstractRunner>  abstractRunners =  generateRunner(PluginType.WRITER);
            String chennelGroupKey =UUID.randomUUID().toString();
            LOG.info("abstractRunners size ={}",abstractRunners.size());
            for(AbstractRunner abstractRunner:abstractRunners){
            	writerRunner =(WriterRunner) abstractRunner;
            	writerRunner.setChannelKey(UUID.randomUUID().toString());
            	LOG.info("set writerRunner channel key ={} for {}",writerRunner.getChannelKey(),abstractRunner.getWriterName());
            	 Channel channel = ClassUtil.instantiate(channelClazz,
                         Channel.class, configuration,writerRunner.getChannelKey());
                 channel.setCommunication(this.taskCommunication);
                 CoreConstant.CHANNELCACHE.put(writerRunner.getChannelKey(), channel);
                 TaskPluginCollector pluginCollector = ClassUtil.instantiate(
            	...
            	writerRunner.setTaskPluginCollector(pluginCollector);
            	Thread writerThread = new Thread(writerRunner,
                        String.format("%d-%d-%d-writer",
                                jobId, taskGroupId, this.taskId));
                //通过设置thread的contextClassLoader，即可实现同步和主程序不通的加载器
            	
            	writerThread.setContextClassLoader(LoadUtil.getJarLoader(
                        PluginType.WRITER, writerRunner.getWriterName()));
            	
            	writerThreads.add(writerThread);
            	this.channelGroupKeyList.add(writerRunner.getChannelKey());
            }
            LOG.debug("channelGroupKeyList ={}",this.channelGroupKeyList.toString());
             ...
             * 生成readerThread
             */
            readerRunner = (ReaderRunner)( generateRunner(PluginType.READER,transformerInfoExecs).get(0));
        ...
        public void doStart() {
        	for (Thread  thread:writerThreads){
                ...
                if (!thread.isAlive() || this.taskCommunication.getState() == State.FAILED) {
                    throw DataXException.asDataXException(
        ...
        public List<AbstractRunner> generateRunner(PluginType pluginType) {
            return generateRunner(pluginType, null);
        }

        public List<AbstractRunner> generateRunner(PluginType pluginType, List<TransformerExecution> transformerInfoExecs) {
            ...
            switch (pluginType) {
                case READER:
                    ...
                    pluginCollector = ClassUtil.instantiate(
                            ...
                            configuration, this.taskCommunication,
                            PluginType.READER);
                    List<RecordSender> recordSenders=new ArrayList<RecordSender>();
                    RecordSender recordSender;
                    if (transformerInfoExecs != null && transformerInfoExecs.size() > 0) {
                        for(String channelkey:CoreConstant.CHANNELCACHE.keySet()){
                            if(this.channelGroupKeyList.contains(channelkey)){
                                recordSender = new BufferedRecordTransformerExchanger(taskGroupId, this.taskId, CoreConstant.CHANNELCACHE.get(channelkey),this.taskCommunication ,pluginCollector, transformerInfoExecs);
                                recordSenders.add(recordSender);
                            }
...
//                              只接受本实例的channelkey
                              if(this.channelGroupKeyList.contains(channelkey)) {
                                  Channel channel =CoreConstant.CHANNELCACHE.get(channelkey);
                                  LOG.info("reader runner init recorder sender channel {}",channel.getChannelKey());
                                  recordSender = new BufferedRecordExchanger(channel, pluginCollector);
                                  recordSenders.add(recordSender);
                              }
                    ...
                    ((ReaderRunner) newRunner).setRecordSender(recordSenders);
                    ...
                    break;
                case WRITER:
                	String writerArrStr = this.taskConfig.getString(CoreConstant.WRITER);
                	if(writerArrStr.startsWith("[")&&writerArrStr.endsWith("]")){
                		JSONArray array= JSONArray.parseArray(writerArrStr);
                		for(int i=0;i<array.size();i++){
                			JSONObject jsonObject = JSONObject.parseObject(array.getString(i));
                            LOG.info("init writer ruuner {}",jsonObject.getString(CoreConstant.NAME));
                			 newRunner = LoadUtil.loadPluginRunner(pluginType,
                					 jsonObject.getString(CoreConstant.NAME));
//                			 LOG.info("",);
//                			 // 设置task 的job conf
                             newRunner.setJobConf(Configuration.from(jsonObject.getString(CoreConstant.PARAMETER)));
                             /**
                              * 设置taskPlugin的collector，用来处理脏数据和job/task通信
                              */
                             newRunner.setTaskGroupId(taskGroupId);
                             newRunner.setTaskId(this.taskId);
                             newRunner.setRunnerCommunication(this.taskCommunication);
                             newRunner.setWriterName(jsonObject.getString(CoreConstant.NAME));
                             runners.add(newRunner);
                             writerRunners.add((WriterRunner) newRunner);
                		}
                	}
                    break;
                default:
        ...
        private boolean isShutdown(){
        	boolean flag=true;
        	for(Thread writerThread:writerThreads){
        		flag=!writerThread.isAlive();
        	}
            return !readerThread.isAlive() && flag;
        }
    }
}
