...
public class HdfsWriter extends Writer {
    public static class Job extends Writer.Job {
        ...
        public void prepare() {
            ...
            String addPartitionSql = String.format("alter table %s.%s add IF NOT EXISTS partition(%s='%s')",
                    this.writerSliceConfig.get(Key.HIVE_DB),
                    this.writerSliceConfig.get(Key.HIVE_TABLE),
                    this.writerSliceConfig.get(Key.HIVE_TABLE_PARTITION_COLUMN),
                    this.writerSliceConfig.get(Key.HIVE_TABLE_PARTITION_VALUE));
            LOG.info(" addPartitionSql = {}",addPartitionSql);

//            String addPartitionSql = "alter table bdp_tmp.dm_t_ai_math_datax_test add IF NOT EXISTS partition(data_dt=2021-09-11)";
            int res = HiveHelper.executeHiveCommandE(addPartitionSql);
            LOG.info("execute addPartitionSql  result ={}",res);
    ...
    public static class Task extends Writer.Task {
        ...
        public void init() {
            ...
            this.writerSliceConfig = this.getPluginJobConf();
//            String  isExist=writerSliceConfig.getString(Key.INTERNAL);
//        	if(!StringUtils.isEmpty(isExist)){
//        		writerSliceConfig=Configuration.from(JSONObject.parseObject((writerSliceConfig.getString(Key.INTERNAL))));
//        	}
