...
public class Configuration {
	...
	public Configuration getConfiguration(final String path,String wirterName) {
		Object object = this.get(path);
		if (null == object) {
			return null;
		}
		if(object.toString().startsWith("[")&&object.toString().endsWith("]")){
			...
			for(int i=0;i<array.size();i++){
				...
				if(wirterName.equalsIgnoreCase(jsonObject.getString("name"))){
					return 	Configuration.from(Configuration.toJSONString(jsonObject.get("parameter")));
				}
			}
		}
		return null;
	...
	public String[] getStrings(final String path) {
		...
		if(string.toString().startsWith("[")&&string.toString().endsWith("]")){
			JSONArray  array = JSONArray.parseArray(string.toString());
			for(int i=0;i<array.size();i++){
				JSONObject  jsonObject =JSONObject.parseObject(array.getString(i));
